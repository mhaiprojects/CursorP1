# POE1

Minimal Path of Exile 1 reference project: cloned open-source tools, documentation, and budget build guides.

## Structure

```
docs/           Documentation
repos/          Cloned POE tools (Path of Building, poe.ninja clients)
POE1Builds/     Budget build recommendations
```

## Quick Links

- [Path of Building overview](docs/path-of-building.md)
- [Cloned repos](repos/README.md)
- [Build recommendations](POE1Builds/README.md) — league starters + endgame 500M+ PoB builds

## Re-clone Tools

```bash
cd repos
git clone --depth 1 https://github.com/PathOfBuildingCommunity/PathOfBuilding.git path-of-building
git clone --depth 1 https://github.com/infernumx/poe_ninja_client.git poe-ninja-client
git clone --depth 1 https://github.com/ayberkgezer/poe-api-manager.git poe-api-manager
```
